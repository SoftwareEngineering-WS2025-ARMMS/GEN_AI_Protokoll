...
class DataBaseConnection:
    ...
    def __init__(self, db_name:str, user_name:str, password:str, host:str, port:int):
        self._DB_CONFIG = {'dbname': db_name, 'user': user_name, 'password': password, 'host': host, 'port': port}
    ...
    def save_protocol(self, protocol: dict, user_id: str) -> bool:
        try:
            ...
            query_1 = ('INSERT INTO ProtocolMetadata (title, date, place, user_id, numberOfAttendees) '
                       'VALUES (%s, %s, %s, %s, %s)')
            cursor.execute(query_1, vars=(protocol['title'], protocol['date'], protocol['place'],
                                          user_id, protocol['numberOfAttendees']))
            cursor.close()
            cursor = self.connection.cursor()
            cursor.execute('''SELECT MAX(id) FROM protocolmetadata WHERE user_id= %s
                                AND title = %s AND place = %s AND numberOfAttendees = %s''',
                                vars=(user_id, protocol['title'], protocol['place'], protocol['numberOfAttendees']))
            protocol_id = cursor.fetchone()[0]
            cursor.close()
            n = len(protocol['agendaItems'])
            query_2 ='INSERT INTO AgendaItem (id, title, explanation) VALUES (%s, %s, %s)' + ', (%s, %s, %s)' * (n - 1)
            cursor = self.connection.cursor()
            parameters = list(sum([(protocol_id, protocol['agendaItems'][i]['title'], protocol['agendaItems'][i]['explanation']) for i in range(n)], ()))
    ...
    def get_protocol_by_id(self, protocol_id: int, user_id: str) -> dict:
        cursor = self.connection.cursor()
        query_1 = 'SELECT title, date, place, numberofattendees FROM ProtocolMetadata WHERE id = %s AND user_id = %s;'
        ...
        cursor.close()
        return {'title': protocol_metadata[0],
    ...
    def get_protocol_summaries(self, user_id: str) -> list:
        ...
        cursor.close()
        cursor = self.connection.cursor()
        query_2 = "SELECT p.id, a.title FROM protocolmetadata p, agendaitem a WHERE user_id = %s AND p.id = a.id;"
