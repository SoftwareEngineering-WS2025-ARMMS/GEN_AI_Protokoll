...
def test_recording():
    def intern(n: int, recording: Recording, full_length = True) :
        assert n in [1,2,3], "There is no audio file other than 1, 2 and 3 to test."
        ...
        if full_length:
            match n:
                case 1:
                    assert abs(recording.content.duration_seconds - 4.096) < 1e-8
                case 2:
                    assert abs(recording.content.duration_seconds - 5.104) < 1e-8
                case 3:
                    assert abs(recording.content.duration_seconds - 3.456) < 1e-8
    return intern
...
def test_trim_recording(n: int, test_recording):
    try:
        ...
        assert False, f"This test does not run unless the test_from_file_path passes for n = {n}."
    recording = Recording.from_file_path(f"{__test_dir__}/audio_files/test_de_{n}.wav")
    recordings = recording.trim_recording(annotation=[('A', 0.1, 0.5), ('B', 0.8, 1.3), ('C', 2.1, 3.4)])
